...
const form = document.getElementById('form-anexo-viii-c');
...
const tipoAlteracaoSelect = document.getElementById('tipo_alteracao');
const outraAlteracaoWrapper = document.getElementById('outra-alteracao-wrapper');
const outraAlteracaoInput = document.getElementById('outra_alteracao_texto');

tipoAlteracaoSelect.addEventListener('change', () => {
  const ehOutra = tipoAlteracaoSelect.value === 'Outra alteração do Contrato Social';
  outraAlteracaoWrapper.hidden = !ehOutra;
  if (!ehOutra) {
    outraAlteracaoInput.value = '';
    clearError('outra_alteracao_texto');
  }
});
...
function validateForm() {
  ...
  const tipoAlteracao = tipoAlteracaoSelect.value;
  if (!tipoAlteracao) {
    setError('tipo_alteracao', 'Selecione o tipo de alteração.');
    valid = false;
  } else {
    clearError('tipo_alteracao');
  }

  if (tipoAlteracao === 'Outra alteração do Contrato Social' && !outraAlteracaoInput.value.trim()) {
    setError('outra_alteracao_texto', 'Descreva a alteração.');
    valid = false;
  } else {
    clearError('outra_alteracao_texto');
  }
...
form.addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    outra_alteracao_texto: outraAlteracaoInput.value.trim() || null,
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/cadastro/anexo-viii-c`, {
      method: 'POST',
    ...
    form.reset();
    outraAlteracaoWrapper.hidden = true;
  } catch (error) {
    showFeedback(
