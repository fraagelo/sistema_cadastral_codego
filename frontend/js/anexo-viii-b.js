...
const form = document.getElementById('form-anexo-viii-b');
...
function validateForm() {
  ...
  if (!tipoOperacao) {
    setError('tipo_operacao', 'Selecione o tipo de operação.');
    valid = false;
  } else {
    clearError('tipo_operacao');
  }
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/cadastro/anexo-viii-b`, {
      method: 'POST',
